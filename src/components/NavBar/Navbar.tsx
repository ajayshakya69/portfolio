...
function NavItems() {
  return (
    <ul className="nav-ul">
      {navLinks.map((e) => (
        <li className="nav-li hover:text-neutral-400 transition-all" key={e.id}>
          <a className="nav-li_a " href={e.href}>
            {e.name}
          </a>
        </li>
      ))}
    ...
    <nav className="fixed w-full     top-0 left-0  z-50 bg-black/90 ">
      <div className="max-w-7xl mx-auto flex items-center">
        <div className="py-5 flex justify-between w-full c-space  ">
          <a
            href="/"
            className="text-neutral-400 font-bold text-xl hover:text-white transition-all "
          >
            Ajay
          </a>
          <button
      ...
      <div
        className={"nav-sidebar " + (isMenuOpen ? " max-h-screen" : "max-h-0")}
      >
        <div className=" p-5">
          <NavItems />
        </div>
